...
from bot.components.base import Session, Base, engine
...
for i in range(1):  # extend here
    events_block = get_events_block(i)
    for event_block in events_block:
        e = prepare_event_info(event_block)
        stored_event = session.query(Event).filter(Event.event_url == e[0]).first()
